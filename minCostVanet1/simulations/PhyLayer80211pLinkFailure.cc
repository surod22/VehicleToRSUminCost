 ...
 * and modifications by Susana Rodriguez
 */

#include "PhyLayer80211pLinkFailure.h"
...
Define_Module(PhyLayer80211pLinkFailure);
...
void PhyLayer80211pLinkFailure::initialize(int stage) {
...
AnalogueModel* PhyLayer80211pLinkFailure::getAnalogueModelFromName(std::string name, ParameterMap& params) {
...
AnalogueModel* PhyLayer80211pLinkFailure::initializeLogNormalShadowing(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211pLinkFailure::initializeJakesFading(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211pLinkFailure::initializeBreakpointPathlossModel(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211pLinkFailure::initializeTwoRayInterferenceModel(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211pLinkFailure::initializeSimplePathlossModel(ParameterMap& params){
...
AnalogueModel* PhyLayer80211pLinkFailure::initializePERModel(ParameterMap& params) {
    double per = params["packetErrorRate"].doubleValue();
    return new PERModel(per);
}

Decider* PhyLayer80211pLinkFailure::getDeciderFromName(std::string name, ParameterMap& params) {
    if(name == "Decider80211pLinkFailure") {
        protocolId = IEEE_80211;
        return initializeDecider80211pLinkFailure(params);
    }
    return BasePhyLayer::getDeciderFromName(name, params);
}

AnalogueModel* PhyLayer80211pLinkFailure::initializeSimpleObstacleShadowing(ParameterMap& params){
...
Decider* PhyLayer80211pLinkFailure::initializeDecider80211pLinkFailure(ParameterMap& params) {
    ...
    Decider80211pLinkFailure* dec = new Decider80211pLinkFailure(this, sensitivity, centerFreq, findHost()->getIndex(), collectCollisionStatistics, coreDebug);
    dec->setPath(getParentModule()->getFullPath());
    return dec;
}

void PhyLayer80211pLinkFailure::changeListeningFrequency(double freq) {
    Decider80211pLinkFailure* dec = dynamic_cast<Decider80211pLinkFailure*>(decider);
    assert(dec);
    dec->changeFrequency(freq);
}
void PhyLayer80211pLinkFailure::handleSelfMessage(cMessage* msg) {
...
AirFrame *PhyLayer80211pLinkFailure::encapsMsg(cPacket *macPkt)
{
...
int PhyLayer80211pLinkFailure::getRadioState() {
